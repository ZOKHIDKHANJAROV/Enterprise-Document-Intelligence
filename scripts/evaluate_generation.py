...
def main():
    ...
    count = len(rows)
    summary = {'examples': count, 'exact_match': exact / count if count else 0.0, 'mean_token_recall': sum(scores) / count if count else 0.0, 'warning': 'Lexical metrics are a baseline only.'}
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps({'summary': summary, 'examples': rows}, ensure_ascii=False, indent=2), encoding='utf-8')
